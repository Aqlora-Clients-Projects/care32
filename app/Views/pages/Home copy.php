...
<section class="dentora-hero">
  ...
  <?= view('includes/navbar'); ?>
        <div class="hero-content-inner">
            ...
            <a href="<?= base_url('contact') ?>" class="hero-cta hero-anim">
                Book a Appointment <i class="fa-solid fa-arrow-right"></i>
            </a>

            <div class="hero-mini-card hero-anim">
                <img id="hero-mini-img" src="<?= base_url('assets/images/services/cosmetic_dentistry.png') ?>" class="mini-img" alt="Dental Care">
...
<!-- Scroll List -->
<div class="container services-scroll d-none d-md-flex" id="stepper-container">
...
<script>
    (function() {
        ...
        const buttonFillDuration = 2.5000; // 2 sec for button to fill
        const waitAfterButton = 2000; // 6 sec wait after button is filled
        const lineFillDuration = 2000; // 2 sec for line to fill
        const cyclePause = 50000; // 10-Minute delay before restarting the cycle
        
        function animateStepper() {
            // Reset everything if we reached the end
            if (currentIndex >= items.length) {
                setTimeout(() => {
                    ...
                    // Small delay to allow CSS to apply instantly without transition
                    setTimeout(() => {
                        items.forEach(item => item.classList.remove("no-transition"));
                        lines.forEach(line => line.classList.remove("no-transition"));
                        if (items[0]) items[0].classList.add("active");
                        currentIndex = 0;
                        setTimeout(animateStepper, buttonFillDuration + waitAfterButton);
                    }, 50);
                }, cyclePause); // 10-Minute delay
                return;
            }
            
            // Activate current item (Starts the 3sec button fill)
            if (items[currentIndex]) items[currentIndex].classList.add("active");
            
            if (currentIndex < lines.length) {
                // Wait for button to fill (3s) + the extra wait (10s) before filling next line
                setTimeout(() => {
                    if (lines[currentIndex]) lines[currentIndex].classList.add("filled");
                    
                    // After line is filled (3s), move to next item
                    setTimeout(() => {
                        currentIndex++;
                        animateStepper();
                    }, lineFillDuration);
                ...
                // Just wait for it to fill before triggering the 10-minute cycle pause
                setTimeout(() => {
                    currentIndex++;
                    animateStepper();
                }, buttonFillDuration);
        ...
        if (items.length > 0) {
            // Wait for first button fill + wait time before triggering next
            setTimeout(animateStepper, buttonFillDuration + waitAfterButton);
        }
    })();
</script>
    ...
    <div class="container">
        ...
        <div class="services-header-wrapper d-flex justify-content-between align-items-end mb-5">
            ...
            <div class="services-header-right d-none d-md-block">
                <a href="<?= base_url('contact') ?>" class="btn btn-primary px-4 py-2" style="border-radius: 8px; font-weight: 600; background-color: #5c67f2; border: none;">Book an Appointment</a>
            </div>
        </div>
        ...
        <div class="text-center mt-5" id="loadMoreContainer">
            <button class="btn btncolor " id="loadMoreBtn" style="border-radius: 50px; font-weight: 600;">Load More</button>
        </div>
    </div>
</section>
        ...
        <div class="booking-container-card">
            ...
            <div class="col-lg-6" data-aos="fade-left" data-aos-duration="1000">
                <div class="booking-form-wrapper p-4 p-md-5">
                    <h3 class="mb-4" style="font-weight: 700; color: #111;">Book Appointment</h3>
                    <form action="<?= base_url('contact/submit') ?>" method="POST" class="booking-form">
                        ...
                        <button type="submit" class="btn btncolor w-100 py-3" style="font-size: 18px; border-radius: 50px; font-weight: 600;">Confirm Appointment</button>
                    </form>
                </div>
            </div>
            </div>
